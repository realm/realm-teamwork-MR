...
class TaskViewController: FormViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        teams = realm.objects(Team.self).sorted(byKeyPath: "name", ascending:true)
        ...
        if newTaskMode {
            ...
            task = tasksRealm!.objects(Task.self).filter("id = %@", taskId!).first
        }
        
        // See if this is a new task, or viewing/editing an exsisting one:
        if newTaskMode { // new one!
    ...
    @IBAction func BackCancelPressed(sender: AnyObject) {
        ...
        if (titleRow?.value?.isEmpty == false && titleRow?.value != task!.title) || descriptionRow?.value?.isEmpty == false && descriptionRow?.value != task!.taskDescription {
        ...
        let form =
            TextRow("Task Title") { row in
                ...
                +++ Section("Work Location") { section in
                    section.header = {
                        var header = HeaderFooterView<UIView>(.callback({
                            ...
                            let coordinate = self.taskCoordinate(task: task)
                ...
                <<< PushRow<String>() { row in
                    ...
                    let teamIds:[String] = realm.objects(Team.self).map{$0.id}
                    ...
                    if let assignedTeam = Team.getTeamForID(id: task!.team) {
                        ...
                        if let id = row.value {
                            ...
                            let newTeam = aRealm.objects(Team.self).filter(NSPredicate(format: "id = %@", id)).first
                            ...
                            if oldTeamId != nil && oldTeamId != newTeam!.id {
                                let oldTeamRealm = aRealm.objects(Team.self).filter(NSPredicate(format: "id = %@", oldTeamId!)).first
        ...
        if let workSection = self.form.sectionBy(tag: "Work Location") {
            ...
            if let latitude = placemark.location?.coordinate.latitude, let longitude = placemark.location?.coordinate.longitude {
                ...
                try!  realm.write {
                    ...
                    theLocation.streetAddress = placemark.addressDictionary?["Street"] as? String
                    theLocation.city = placemark.addressDictionary?["City"] as? String
                    theLocation.stateProvince = placemark.addressDictionary?["State"] as? String
                    theLocation.countryCode = placemark.addressDictionary?["CountryCode"] as? String
                    theLocation.postalCode = placemark.addressDictionary?["ZIP"] as? String
                    theLocation.haveLatLon = true
                    theLocation.lookupStatus = 0
                    
                    realm.add(theLocation, update: isNewLocation)
                    
                }
                // and update the task itself
                try! tasksRealm?.write {
                    tasksRealm?.add(self.task!, update: true)
                }
            }
